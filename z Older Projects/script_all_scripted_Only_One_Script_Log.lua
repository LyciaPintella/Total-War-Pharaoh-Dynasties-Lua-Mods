...
-- string.find remapping
-- This remapping sets the final 'plain' argument of string.find to be true by default. This means that pattern matching (a slightly less powerful version of regular expressions)
-- is turned OFF by default. Client scripts calling string.find can turn it on by explicitly supplying false for this parameter.
-- This was done because since the beginning of time we have used our own in-house string.find function which supports utf8 strings, but which does not support pattern-matching.
-- Now, we have switched them back, so that we use lua's internal string.find function (which, in addition to providing pattern-matching, is also faster). However, there are lots
-- of string.find uses in the existing script base that are now broken, as they are inadvertently using pattern special characters. Since it seems likely that in most use-cases
-- we won't want to activate pattern matching, we turn it off by default here.
local find = string.find;
function string.find(str, search_term, start_index, plain)
     if plain ~= false then
          return find(str, search_term, start_index, true);
     end;

     return find(str, search_term, start_index, false);
end;

-- also expose the original find function at string.find_lua
string.find_lua = find;
...
function script_error(msg, stack_level_modifier, suppress_assert)
     ...
     if stack_level_modifier then
          if not type(stack_level_modifier) == "number" then
               script_error("ERROR: script_error() called but supplied stack level modifier [" ..
               tostring(stack_level_modifier) .. "] is not a number");
               return false;
          end;
     else
...
do
     ...
     local file_str = common.filesystem_lookup("/script/", "enable_console_logging");
     if (file_str ~= "") or (tweaker_value ~= "0" and tweaker_value ~= "") then
          __write_output_to_logfile = true
     end
end;
__write_output_to_logfile = true; --Added for mod
__logfile_path = "";
...
if __write_output_to_logfile then
     ...
     if not file then
          ...
          script_error("ERROR: tried to create logfile with filename " ..
          filename .. " but operation failed with error: " .. tostring(err_str));
     else
          file:write("\n");
...
--- @desc <li>narrative</li>
--- @desc <li>savegame</li>
--- @desc <li>scripted_tours</li>
--- @desc <li>status</li>
--- @desc <li>traits</li>
--- @desc <li>ui</li></ul>
--- @desc
...
function remap_outputs(out_impl, suppress_new_session_output)
     ...
     function out.inc_tab(func_name)
          ...
          if not current_tab_level then
               script_error("ERROR: inc_tab() called but supplied output function name [" ..
               tostring(func_name) .. "] not recognised");
               return false;
          end;
     ...
     function out.dec_tab(func_name)
          ...
          if not current_tab_level then
               script_error("ERROR: dec_tab() called but supplied output function name [" ..
               tostring(func_name) .. "] not recognised");
               return false;
          end;
     ...
     function out.cache_tab(func_name)
          ...
          if not current_tab_level then
               script_error("ERROR: cache_tab() called but supplied output function name [" ..
               tostring(func_name) .. "] not recognised");
               return false;
          end;
     ...
     function out.restore_tab(func_name)
          ...
          if not cached_tab_level then
               script_error(
               "ERROR: restore_tab() called but could find no cached tab value for supplied output function name [" ..
               tostring(func_name) .. "]");
               return false;
          end;
...
function add_event_callback(event, callback, user_defined_list)
     if type(event) ~= "string" then
          script_error("ERROR: add_event_callback() called but supplied event [" ..
          tostring(event) .. "] is not a string");
          return false;
     end;
     ...
     if type(callback) ~= "function" then
          script_error("ERROR: add_event_callback() called but supplied callback [" ..
...
function clear_event_callbacks(user_defined_list)
     if not type(user_defined_list) == "table" then
          script_error("ERROR: clear_event_callbacks() called but supplied user defined list [" ..
          tostring(user_defined_list) .. "] is not a table");
          return false;
     end;
